...
************************************************************************
      ...
      INTEGER NFAIL(NFL),IFAIL,I,IDIM,TOT,ITOT,NTOT
      INTEGER M1FLAG,M2FLAG,M3FLAG,MHDFLAG,MHUFLAG
      INTEGER MSFLAG,AKFLAG,ALFLAG,OMGFLAG,MAFLAG
      INTEGER NL,NK,NTB,NMU,NMUP,NMSP,NM3H,N1,N2,N3,N4,NM1,NM2,NM3
      INTEGER IL,IK,ITB,IMU,IMUP,IMSP,IM3H,I1,I2,I3,I4,IM1,IM2,IM3
...
*   Initialization of the range of parameters that has passed all tests
      ...
      M3HN=MAX(M3HMAX,M3HMIN)
      M3HNN=MIN(M3HMAX,M3HMIN)
...
*   Check for errors
 ...
 29   ERR=0
     ...
     . " CAN BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(KMIN.EQ.0d0 .OR. (NK.GT.1 .AND.(KMAX.EQ.0d0 .OR. 
     . (KMAX.NE.1d99 .AND. KMIN*KMAX.LT.0d0))))THEN
       IF((AKMIN.NE.0d0 .AND. AKMIN.NE.1d99) .OR. (AKMIN.EQ.0d0
     . .AND. AKMAX.NE.1d99 .AND. AKMAX.NE.0d0 .AND. NAK.GT.1))THEN
...
*   Number of points
     ...
     . NL.LE.0 .OR. NK.LE.0 .OR. N1.LE.0 .OR. N2.LE.0
     . .OR. N3.LE.0 .OR. N4.LE.0 .OR. NMU.LE.0 .OR. NMUP.LE.0
     . .OR. NMSP.LE.0 .OR. NM3H.LE.0)THEN
...
*   Warnings
      ...
      IF(M3HMIN.EQ.M3HMAX .AND. NM3H.GT.1)THEN
       WRITE(0,1)"WARNING M3H^2_min = M3H^2_max => N3H=1"
       NM3H=1
      ENDIF
      ...
      IF(M3HMIN.NE.M3HMAX .AND. NM3H.EQ.1)THEN
       WRITE(0,10)"WARNING NM3H=1 => M3H^2_max=M3H^2_min=",M3HMIN
       M3HMAX=M3HMIN
      ENDIF
      ...
      IF(MAFLAG/3.EQ.1)THEN
       IF(MPMIN.EQ.MPMAX .AND. NMP.GT.1)THEN
        WRITE(0,1)"WARNING MP_min=MP_max => NMP=1"
        NMP=1
        N3=NMP
       ENDIF
       IF(XISMIN.EQ.XISMAX .AND. NXIS.GT.1)THEN
       ...
       IF(MPMIN.NE.MPMAX .AND. NMP.EQ.1)THEN
        WRITE(0,10)"WARNING NMP=1 => MP_max=MP_min=",MPMIN
        MPMAX=MPMIN
       ENDIF
       IF(XISMIN.NE.XISMAX .AND. NXIS.EQ.1)THEN
        WRITE(0,10)"WARNING NXIS=1 => XIS_max=XIS_min=",XISMIN
...
*********************************************************************      
      ...
      INTEGER NSUSY,NGUT,NMES,IMAX,JMAX
...
*
     ...
     . MQGUT,MUGUT,MDGUT,ML3GUT,ME3GUT,MLGUT,MEGUT
      ...
      IF(IFAIL.EQ.0.OR.IFAIL.GE.10)THEN
       ...
       DO I=1,3
        RES(IDIM,15+2*I)=SMASS(I)
        RES(IDIM,16+2*I)=SCOMP(I,3)**2
       ENDDO
       DO I=1,2
        RES(IDIM,21+2*I)=PMASS(I)
        RES(IDIM,22+2*I)=PCOMP(I,2)**2
       ENDDO
       RES(IDIM,27)=CMASS
       RES(IDIM,28)=DABS(MNEU(1))
       RES(IDIM,29)=NEU(1,1)**2
       RES(IDIM,30)=NEU(1,3)**2+NEU(1,4)**2
       RES(IDIM,31)=NEU(1,5)**2
       RES(IDIM,32)=OMG
