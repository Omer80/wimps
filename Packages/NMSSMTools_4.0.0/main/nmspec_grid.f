...
************************************************************************
      ...
      INTEGER ITER,ITERMU,MUFLAG,SFFLAG,Q2FIX,IM,IGUESS
...
*   Guess parameters at Q2/MGUT
      ...
      IF(IGUESS.EQ.0 .AND. (IFAIL.EQ.0 .OR. IFAIL.EQ.10))THEN
       IGUESS=1
       IF(Q2FIX.EQ.0)THEN
...
*******************************************************************
      ...
      INTEGER N0,NLOOP,NBER,NPAR,ERR,NTOT
...
*   Warnings
      ...
      IF(M0MIN.EQ.M0MAX .AND. NM0.GT.1)THEN
       WRITE(0,1)"WARNING M0_min=M0_max => NM0=1"
       NM0=1
      ENDIF
      IF(M12MIN.EQ.M12MAX .AND. NM12.GT.1)THEN
       WRITE(0,1)"WARNING M12_min=M12_max => NM12=1"
       NM12=1
      ENDIF
      IF(A0MIN.EQ.A0MAX .AND. NA0.GT.1)THEN
       WRITE(0,1)"WARNING A0_min=A0_max => NA0=1"
       NA0=1
      ENDIF
      ...
      IF(M3HMIN.EQ.M3HMAX .AND. NM3H.GT.1)THEN
       WRITE(0,1)"WARNING M3H^2_min=M3H^2_max => NM3H=1"
       NM3H=1
      ENDIF

      IF(M0MIN.NE.M0MAX .AND. NM0.EQ.1)THEN
       WRITE(0,10)"WARNING NM0=1 => M0_max=M0_min=",M0MIN
       M0MAX=M0MIN
      ENDIF
      IF(M12MIN.NE.M12MAX .AND. NM12.EQ.1)THEN
       WRITE(0,10)"WARNING NM12=1 => M12_max=M12_min=",M12MIN
       M12MAX=M12MIN
      ENDIF
      IF(A0MIN.NE.A0MAX .AND. NA0.EQ.1)THEN
       WRITE(0,10)"WARNING NA0=1 => A0_max=A0_min=",A0MIN
       A0MAX=A0MIN
      ENDIF
      ...
      ELSE
       IF(MHDMIN.EQ.MHDMAX .AND. NMHD.GT.1)THEN
        WRITE(0,1)"WARNING MHD^2_min=MHD^2_max => NMHD=1"
        NMHD=1
        N3=NMHD
       ENDIF
       IF(MHUMIN.EQ.MHUMAX .AND. NMHU.GT.1)THEN
        WRITE(0,1)"WARNING MHU^2_min=MHU^2_max => NMHU=1"
        NMHU=1
        N4=NMHU
       ENDIF
       IF(MHDMIN.NE.MHDMAX .AND. NMHD.EQ.1)THEN
        WRITE(0,10)"WARNING NMHD=1 => MHD^2_max=MHD^2_min=",MHDMIN
        MHDMAX=MHDMIN
       ENDIF
       IF(MHUMIN.NE.MHUMAX .AND. NMHU.EQ.1)THEN
        WRITE(0,10)"WARNING NMHU=1 => MHU^2_max=MHU^2_min=",MHUMIN
        MHUMAX=MHUMIN
       ENDIF
      ENDIF
