...
*    (check of pos. masses^2, and for the mixing matrices)
      ...
      CALL jacobi(MSfull,3,3,ms2,OS,nrot)

      CALL jacobi(MPfull,3,3,ma2,OP,nrot)
 ...
 500    continue
     ...
     .         mtau/v1,v1,v2,pa,Q,piPP)
        ...
        CALL jacobi(MSfull,3,3,ms2cor,OS0,nrot)
    
        CALL jacobi(MPfull,3,3,ma2cor,OP0,nrot)
     ...
     .         +abs(pa**2-abs(ma2cor(kk)))/pa**2.gt.1d-5) then
...
* UE: Estimate 2-loop corrs. borrowed from the CP-odd Higgs:
 ...
 600   continue
     ...
     .        + sb**2*tadS(1)/v1/sqrt(2d0) + cb**2*tadS(2)/v2/sqrt(2d0)
        ...
        if(abs(phc**2-abs(mhcsqcorr))/phc**2.gt.1d-5)then
...
c     now take the square root (if allowed)
...
* mod. by UE: resum large logs ~ht^2*ln(Q^2/mt^2) (not included in dmt):
       ...
       mtmssm=mtmssm*(1d0-9d0*htma**2/(8d0*pi*asma)*
     .    (1d0-dlqa))**(-1d0/6d0)
