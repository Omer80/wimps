...
static double Spectra22A(char*name1,char*name2,double ** Spectra,txtList plusA)
{
  ...
  { 
     ...
     *(cc23->interface->gswidth)=1;
     ...
     for(i=0;i<5;i++) cc23->interface->pinf(1,i+1,pmass+i,code+i);
     ...
     {  char buff[100];
       ...
       { double dSigmaDz=0,x=exp(Zi(i));
         ...
         { dSigmaDz=x*Mcdm*dSigmadERest(x*Mcdm);
           ...
           x2Sum+=(Zi(i)-Zi(i+1))*x*dSigmaDz;
         }else  Spectra[0][i]+=dSigmaDz;
       }
       sprintf(cRec->txt+strlen(cRec->txt) ,"| %E",x2Sum);
...
//  float mi[NEn]={10,25,50,80.3,91.2,100,150,176,200,250,350,500,750,1000,1500,2000,3000,5000};
   ...
   switch(CHin)
   { 
     case 14: case 15: case 16: case 17: if(i0<5) i0=6;  for(l=0;l<NZ;l++) tab[l]=phidiff[CHin][CHout][i0][l]; return;
     case 10: case 11: case 12: case 13: if(i0<7) i0=9;  for(l=0;l<NZ;l++) tab[l]=phidiff[CHin][CHout][i0][l]; return;
   } 
...
/*        if(PrintOn && outP==0 ) printf(" %.0f%% transverse W produced\n",100*fabs(left+right));*/
  ...
  { char* nn[2];
    ...
    { p2=0;
      if(N1==23 || abs(N2)==24) m1=M-m2; else m2=M-m1;
    } 
    ...
    { double tabAux[NZ];
      ...
      { double dY=log(M/E[k]/2);
        ...
        for(i=0;i<NZ;i++)tab[i]+=0.5*zInterp(Zi(i)+dY,tabAux);
      }else
      { double w=0;
        numout * d2Proc;
        int l; 
...
//printf("cs23 ok!\n");
      ...
      { double wV2;
        ...
        r*=decayPcmW(2*Mcdm,m[l],m[l_],wV,wV2)/decayPcmW(2*Mcdm,m[l],m[l_],wV,0);
        if(pdg[l]==pdg[l_]) r/=2;
      }
      v_cs[k]=r;                     
    }
...
*/                      
          ...
          if( m_min*m_min -Q1  < QCUT*Mcdm*Mcdm  || m_min*m_min -Q2  < QCUT*Mcdm*Mcdm || 
          (abs(pdg[0])==24 && abs(pdg[1])==24 && pdg[0]*pdg[1]<0 && Mcdm > 500 && v_cs[k]>1.E-3*vcsSum ))
...
//       }
       ...
       { int j; for(j=0;j<4;j++) vSigmaCh[nAnCh].prtcl[j]=N[j];}
       nAnCh++;
       ...
       if(!PlusAok && Spectra[0] && charge3[0])
       {
...
/*        printf("%20.20s  d<x^2>=%.1E\n",buff, x2/x2Sum); */
  ...
  {   
    double c=(1+exp(dmAsymm))/(1+exp(-dmAsymm));
    vcs+=calcSpectrum0(name,name,0,Spectra_,NULL)*0.5*c;
    for(l=0;l<6;l++) if(Spectra[l])for(i=0;i<NZ;i++)Spectra[l][i]+= Spectra_[l][i]*0.5*c;

    c=1/c;
    vcs+=calcSpectrum0(aname,aname,0,Spectra_,NULL)*0.5*c;
    for(l=0;l<6;l++) if(Spectra[l])for(i=0;i<NZ;i++)Spectra[l][i]+= Spectra_[l][i]*0.5*c;
  ...
  if(vcs) for(l=0;l<6;l++) if(Spectra[l])for(i=0;i<NZ;i++)Spectra[l][i]/=vcs;
//  if(strcmp(name,aname)) vcs/=2;


  vSigmaCh[nAnCh].weight=0;
  { int i,j; 
    for(j=0;j<4;j++) vSigmaCh[nAnCh].prtcl[j]=NULL;                                       
    if(vcs)for(i=0;i<nAnCh;i++) vSigmaCh[i].weight/= vcs; 
    for(i=0;i<nAnCh-1;)
  ...
  { int i=0;
    ...
    for(i=0;vSigmaCh[i].weight>0;i++)
    {
    sprintf(txt,"%s,%s -> %s %s", vSigmaCh[i].prtcl[0],
                                  vSigmaCh[i].prtcl[1],
                                  vSigmaCh[i].prtcl[2],
                                  vSigmaCh[i].prtcl[3]);
    printf("  %-20.20s  %.2E\n",txt,vSigmaCh[i].weight*vcs);
    }
  }     
