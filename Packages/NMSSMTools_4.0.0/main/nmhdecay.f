...
*******************************************************************
      ...
      INTEGER I,NLINE,INL,ICH,IX,IVAL,Q2FIX,ERR,NMSFLAG
      INTEGER N0,NLOOP,NBER,NPAR,OMGFLAG,MAFLAG,PFLAG
 ...
 29   ERR=0
     ...
     . " CAN BE GIVEN IN BLOCK EXTPAR"
      ...
      IF(PAR(2).EQ.0d0)THEN
       IF(PAR(6).NE.1d99.AND.PAR(6).NE.0d0)THEN
        WRITE(0,1)"IF KAPPA IS 0, AKAPPA MUST BE 0"
        ERR=1
       ELSEIF(PAR(6).EQ.1d99)THEN
        PAR(6)=0d0
       ENDIF
       IF(PAR(24).NE.1d99.AND.XIS.NE.1d99)THEN
        WRITE(0,1)"EITHER MP OR XIS CAN BE GIVEN IN BLOCK EXTPAR"
       ERR=1
       ENDIF
      ENDIF
...
*********************************************************************      
      ...
      INTEGER I,NBIN,IFAIL,NMSFLAG,OMGFLAG,MAFLAG,PFLAG,Q2FIX
     ...
     . BRBtaunumax,BRBtaunumin
      ...
      DOUBLE PRECISION MHMIN,MHMAX,chi2MAX,DELMB
     ...
     . MQGUT,MUGUT,MDGUT,ML3GUT,ME3GUT,MLGUT,MEGUT
      ...
      COMMON/HIGGSFIT/MHMIN,MHMAX,chi2MAX
      COMMON/DELMB/DELMB
      COMMON/LHCSIG/SIG
...
!      WRITE(0,*)
!      WRITE(0,*)"From microMEGAS:"
!      CALL printHiggs(6)
!      WRITE(0,*)
!      WRITE(0,*)"Mh1_ = ",findValW("Mh1_"),"Mh2_ = ",findValW("Mh2_"),
!     . "Mh3_ = ",findValW("Mh3_")
!      WRITE(0,*)"Mha_ = ",findValW("Mha_"),"Mhb_ = ",findValW("Mhb_")
!      WRITE(0,*)
!      WRITE(0,*)"From NMSSMTools:"
!      WRITE(0,*)"Scalar Higgses (pole | run | eff | D(run-eff)):"
!      WRITE(0,*)"MH1",SMASS(1),MH(1),MMH(1),DMH(1)
!      WRITE(0,*)"MH2",SMASS(2),MH(2),MMH(2),DMH(2)
!      WRITE(0,*)"MH3",SMASS(3),MH(3),MMH(3),DMH(3)
!      WRITE(0,*)
!      WRITE(0,*)"Pseudoscalar Higgses (pole | run | eff | D(run-eff)):"
!      WRITE(0,*)"MA1",AMASS(1),MA(1),MMA(1),DMA(1)
!      WRITE(0,*)"MA2",AMASS(2),MA(2),MMA(2),DMA(2)
!      WRITE(0,*)
!      WRITE(0,*)"Charged Higgs (pole | run | eff | D(run-eff)):"
!      WRITE(0,*)"MHC",CMASS,MHC,MMHC,DMHC
!      WRITE(0,*)
!      WRITE(0,*)" omega h^2 =",OMG
!      omg_=PRINTCHANNELS(Xf,1.D-3,1.D-4,1,ch)
      ...
      WRITE(17,900) 2,"4.0.0      # Version number"
     ...
     . "# Relic density too large (WMAP)"
      IF(PROB(30).LT.0d0.AND.PROB(30).NE.-1d00.AND.PROB(30).NE.-2d0)
     ...
     .  WRITE(17,901) 67,XISSUSY,"XIS"
      ...
      ENDIF
      IF(MUPSUSY.NE.0d0)
     .  WRITE(17,901) 68,MUPSUSY,"MU'"
      IF(MSPSUSY.NE.0d0)
     .  WRITE(17,901) 69,MSPSUSY,"MS'**2"
      IF(M3HSUSY.NE.0d0)
     .  WRITE(17,901) 72,M3HSUSY,"M3H**2"
...
!      IF(OMGFLAG.NE.0)CALL printRelDen(PROB,17)
 ...
 1    WRITE(18,899) "# HIGGS + TOP BRANCHING RATIOS IN SLHA FORMAT"
      ...
      WRITE(18,900) 2,"4.0.0      # Version number"
 ...
 2    IF(OMGFLAG.EQ.0) RETURN
      ...
      WRITE(19,900) 2,"4.0.0      # Version number"
