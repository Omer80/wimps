...
, {"~uL","~UL", 100002, "MSuL","wSuL",0,3,2}
, {"~uR","~UR", 200002, "MSuR","wSuR",0,3,2}
, {"~dL","~DL", 100001, "MSdL","wSdL",0,3,-1}
, {"~dR","~DR", 200001, "MSdR","wSdR",0,3,-1}
, {"~cL","~CL", 100004, "MScL","wScL",0,3,2}
, {"~cR","~CR", 200004, "MScR","wScR",0,3,2}
, {"~sL","~SL", 100003, "MSsL","wSsL",0,3,-1}
, {"~sR","~SR", 200003, "MSsR","wSsR",0,3,-1}
, {"~t1","~T1", 100006, "MSt1","wSt1",0,3,2}
, {"~t2","~T2", 200006, "MSt2","wSt2",0,3,2}
, {"~b1","~B1", 100005, "MSb1","wSb1",0,3,-1}
, {"~b2","~B2", 200005, "MSb2","wSb2",0,3,-1}
};
...
static REAL varValues_[397]={
   1.172000E-01,  9.130000E+01,  8.020000E+01,  1.735000E+02,  4.230000E+00,  1.270000E+00,  1.000000E+02,  3.123000E-01,  1.777000E+00,  5.000000E-02
...
int calcMainFunc(void)
{
  ...
  cErr=1;
   V[39]=V[2]/(V[1]);
   if(!finite(V[39]) || FError) return 39;
   V[40]=sqrt(1-pow(V[39],2));
   if(!finite(V[40]) || FError) return 40;
   V[41]=2*V[40]*V[39];
   if(!finite(V[41]) || FError) return 41;
   V[42]=pow(V[39],2)-pow(V[40],2);
   if(!finite(V[42]) || FError) return 42;
   V[43]=pow(V[7],2)/(pow(2*V[40]*V[2],2))/(M_SQRT2);
   if(!finite(V[43]) || FError) return 43;
   V[44]=slhaVal("MASS",V[1],1,25);
   if(!finite(V[44]) || FError) return 44;
   V[45]=slhaVal("MASS",V[1],1,35);
   if(!finite(V[45]) || FError) return 45;
   V[46]=slhaVal("MASS",V[1],1,37);
   if(!finite(V[46]) || FError) return 46;
   V[47]=slhaVal("MASS",V[1],1,1000022);
   if(!finite(V[47]) || FError) return 47;
   V[48]=slhaVal("MASS",V[1],1,1000023);
   if(!finite(V[48]) || FError) return 48;
   V[49]=slhaVal("MASS",V[1],1,1000025);
   if(!finite(V[49]) || FError) return 49;
   V[50]=slhaVal("MASS",V[1],1,1000035);
   if(!finite(V[50]) || FError) return 50;
   V[51]=slhaVal("MASS",V[1],1,1000024);
   if(!finite(V[51]) || FError) return 51;
   V[52]=slhaVal("MASS",V[1],1,1000037);
   if(!finite(V[52]) || FError) return 52;
   V[53]=slhaVal("MASS",V[1],1,1000021);
   if(!finite(V[53]) || FError) return 53;
   V[54]=slhaVal("MASS",V[1],1,1000012);
   if(!finite(V[54]) || FError) return 54;
   V[55]=slhaVal("MASS",V[1],1,1000014);
   if(!finite(V[55]) || FError) return 55;
   V[56]=slhaVal("MASS",V[1],1,1000016);
   if(!finite(V[56]) || FError) return 56;
   V[57]=slhaVal("MASS",V[1],1,1000011);
   if(!finite(V[57]) || FError) return 57;
   V[58]=slhaVal("MASS",V[1],1,2000011);
   if(!finite(V[58]) || FError) return 58;
   V[59]=slhaVal("MASS",V[1],1,1000013);
   if(!finite(V[59]) || FError) return 59;
   V[60]=slhaVal("MASS",V[1],1,2000013);
   if(!finite(V[60]) || FError) return 60;
   V[61]=slhaVal("MASS",V[1],1,1000015);
   if(!finite(V[61]) || FError) return 61;
   V[62]=slhaVal("MASS",V[1],1,2000015);
   if(!finite(V[62]) || FError) return 62;
   V[63]=slhaVal("MASS",V[1],1,1000001);
   if(!finite(V[63]) || FError) return 63;
   V[64]=slhaVal("MASS",V[1],1,2000001);
   if(!finite(V[64]) || FError) return 64;
   V[65]=slhaVal("MASS",V[1],1,1000002);
   if(!finite(V[65]) || FError) return 65;
   V[66]=slhaVal("MASS",V[1],1,2000002);
   if(!finite(V[66]) || FError) return 66;
   V[67]=slhaVal("MASS",V[1],1,1000003);
   if(!finite(V[67]) || FError) return 67;
   V[68]=slhaVal("MASS",V[1],1,2000003);
   if(!finite(V[68]) || FError) return 68;
   V[69]=slhaVal("MASS",V[1],1,1000004);
   if(!finite(V[69]) || FError) return 69;
   V[70]=slhaVal("MASS",V[1],1,2000004);
   if(!finite(V[70]) || FError) return 70;
   V[71]=slhaVal("MASS",V[1],1,1000005);
   if(!finite(V[71]) || FError) return 71;
   V[72]=slhaVal("MASS",V[1],1,2000005);
   if(!finite(V[72]) || FError) return 72;
   V[73]=slhaVal("MASS",V[1],1,1000006);
   if(!finite(V[73]) || FError) return 73;
   V[74]=slhaVal("MASS",V[1],1,2000006);
   if(!finite(V[74]) || FError) return 74;
   V[75]=sqrt(V[73]*V[74]);
   if(!finite(V[75]) || FError) return 75;
   V[76]=slhaVal("NMIX",V[75],2,1,1);
   if(!finite(V[76]) || FError) return 76;
   V[77]=slhaVal("NMIX",V[75],2,1,2);
   if(!finite(V[77]) || FError) return 77;
   V[78]=slhaVal("NMIX",V[75],2,1,3);
   if(!finite(V[78]) || FError) return 78;
   V[79]=slhaVal("NMIX",V[75],2,1,4);
   if(!finite(V[79]) || FError) return 79;
   V[80]=slhaVal("NMIX",V[75],2,2,1);
   if(!finite(V[80]) || FError) return 80;
   V[81]=slhaVal("NMIX",V[75],2,2,2);
   if(!finite(V[81]) || FError) return 81;
   V[82]=slhaVal("NMIX",V[75],2,2,3);
   if(!finite(V[82]) || FError) return 82;
   V[83]=slhaVal("NMIX",V[75],2,2,4);
   if(!finite(V[83]) || FError) return 83;
   V[84]=slhaVal("NMIX",V[75],2,3,1);
   if(!finite(V[84]) || FError) return 84;
   V[85]=slhaVal("NMIX",V[75],2,3,2);
   if(!finite(V[85]) || FError) return 85;
   V[86]=slhaVal("NMIX",V[75],2,3,3);
   if(!finite(V[86]) || FError) return 86;
   V[87]=slhaVal("NMIX",V[75],2,3,4);
   if(!finite(V[87]) || FError) return 87;
   V[88]=slhaVal("NMIX",V[75],2,4,1);
   if(!finite(V[88]) || FError) return 88;
   V[89]=slhaVal("NMIX",V[75],2,4,2);
   if(!finite(V[89]) || FError) return 89;
   V[90]=slhaVal("NMIX",V[75],2,4,3);
   if(!finite(V[90]) || FError) return 90;
   V[91]=slhaVal("NMIX",V[75],2,4,4);
   if(!finite(V[91]) || FError) return 91;
   V[92]=slhaVal("UMIX",V[75],2,1,1);
   if(!finite(V[92]) || FError) return 92;
   V[93]=slhaVal("UMIX",V[75],2,1,2);
   if(!finite(V[93]) || FError) return 93;
   V[94]=slhaVal("UMIX",V[75],2,2,1);
   if(!finite(V[94]) || FError) return 94;
   V[95]=slhaVal("UMIX",V[75],2,2,2);
   if(!finite(V[95]) || FError) return 95;
   V[96]=slhaVal("VMIX",V[75],2,1,1);
   if(!finite(V[96]) || FError) return 96;
   V[97]=slhaVal("VMIX",V[75],2,1,2);
   if(!finite(V[97]) || FError) return 97;
   V[98]=slhaVal("VMIX",V[75],2,2,1);
   if(!finite(V[98]) || FError) return 98;
   V[99]=slhaVal("VMIX",V[75],2,2,2);
   if(!finite(V[99]) || FError) return 99;
   V[100]=slhaVal("STOPMIX",V[75],2,1,1);
   if(!finite(V[100]) || FError) return 100;
   V[101]=slhaVal("STOPMIX",V[75],2,1,2);
   if(!finite(V[101]) || FError) return 101;
   V[102]=slhaVal("STOPMIX",V[75],2,2,1);
   if(!finite(V[102]) || FError) return 102;
   V[103]=slhaVal("STOPMIX",V[75],2,2,2);
   if(!finite(V[103]) || FError) return 103;
   V[104]=slhaVal("SBOTMIX",V[75],2,1,1);
   if(!finite(V[104]) || FError) return 104;
   V[105]=slhaVal("SBOTMIX",V[75],2,1,2);
   if(!finite(V[105]) || FError) return 105;
   V[106]=slhaVal("SBOTMIX",V[75],2,2,1);
   if(!finite(V[106]) || FError) return 106;
   V[107]=slhaVal("SBOTMIX",V[75],2,2,2);
   if(!finite(V[107]) || FError) return 107;
   V[108]=slhaVal("STAUMIX",V[75],2,1,1);
   if(!finite(V[108]) || FError) return 108;
   V[109]=slhaVal("STAUMIX",V[75],2,1,2);
   if(!finite(V[109]) || FError) return 109;
   V[110]=slhaVal("STAUMIX",V[75],2,2,1);
   if(!finite(V[110]) || FError) return 110;
   V[111]=slhaVal("STAUMIX",V[75],2,2,2);
   if(!finite(V[111]) || FError) return 111;
   V[112]=slhaVal("ALPHA",V[75],0);
   if(!finite(V[112]) || FError) return 112;
   V[113]=slhaVal("HMIX",0,1,2);
   if(!finite(V[113]) || FError) return 113;
   V[114]=deltaMb();
   if(!finite(V[114]) || FError) return 114;
   V[115]=deltaMl();
   if(!finite(V[115]) || FError) return 115;
   V[116]=calcLambdas();
   if(!finite(V[116]) || FError) return 116;
   V[117]=Lambda1();
   if(!finite(V[117]) || FError) return 117;
   V[118]=Lambda3();
   if(!finite(V[118]) || FError) return 118;
   V[119]=Lambda6();
   if(!finite(V[119]) || FError) return 119;
   V[120]=cos(V[112]);
   if(!finite(V[120]) || FError) return 120;
   V[121]=sin(V[112]);
   if(!finite(V[121]) || FError) return 121;
   V[122]=V[113]/(sqrt(1+pow(V[113],2)));
   if(!finite(V[122]) || FError) return 122;
   V[123]=sqrt(1-pow(V[122],2));
   if(!finite(V[123]) || FError) return 123;
   V[124]=pow(V[123],2)-pow(V[122],2);
   if(!finite(V[124]) || FError) return 124;
   V[125]=V[120];

   V[126]=V[121];

   V[127]=-V[121];
   if(!finite(V[127]) || FError) return 127;
   V[128]=V[120];

   V[129]=V[114]/(1+V[114]);
   if(!finite(V[129]) || FError) return 129;
   V[130]=initQCD5(V[0],V[5],V[4],V[3]);
   if(!finite(V[130]) || FError) return 130;
 FirstQ:
 cErr=1;
   V[131]=MbEff(V[6]);
   if(!finite(V[131]) || FError) return 131;
   V[132]=MtEff(V[6]);
   if(!finite(V[132]) || FError) return 132;
   V[133]=McEff(V[6]);
   if(!finite(V[133]) || FError) return 133;
   V[134]=acos(-1);
   if(!finite(V[134]) || FError) return 134;
   V[135]=V[4]*(1+4/(double)((3))*alphaQCD(V[4])/(V[134]));
   if(!finite(V[135]) || FError) return 135;
   V[136]=V[5]*(1+4/(double)((3))*alphaQCD(V[5])/(V[134]));
   if(!finite(V[136]) || FError) return 136;
   V[137]=pow(V[44],2)*pow(V[126],2)+pow(V[45],2)*pow(V[125],2);
   if(!finite(V[137]) || FError) return 137;
   V[138]=pow(V[45],2)*V[125]*V[126]+pow(V[44],2)*V[128]*V[127];
   if(!finite(V[138]) || FError) return 138;
   V[139]=pow(V[44],2)*pow(V[128],2)+pow(V[45],2)*pow(V[127],2);
   if(!finite(V[139]) || FError) return 139;
   V[140]=2*V[119]*V[123]/(V[122])-V[117]*pow(V[123],2)/(pow(V[122],2))-pow(V[7],2)*(pow(V[14]*V[122],2)-V[137])/(pow(2*V[2]*V[40]*V[122],2));
   if(!finite(V[140]) || FError) return 140;
   V[141]=V[140]-1/(double)((2))*pow(V[7],2)*(pow(V[46],2)-pow(V[14],2))/(pow(V[2]*V[40],2));
   if(!finite(V[141]) || FError) return 141;
   V[142]=V[123]/(V[122])*(V[118]+V[141]-V[123]/(V[122])*V[119]-1/(double)((4))*pow(V[7],2)*(V[138]/(V[123])/(V[122])+pow(V[14],2))/(pow(V[2]*V[40],2)));
   if(!finite(V[142]) || FError) return 142;
   V[143]=2*V[142]*V[123]/(V[122])-V[140]*pow(V[123],2)/(pow(V[122],2))-pow(V[7],2)*(pow(V[14]*V[123],2)-V[139])/(pow(2*V[2]*V[40]*V[122],2));
   if(!finite(V[143]) || FError) return 143;
   V[144]=deltaMd();
   if(!finite(V[144]) || FError) return 144;
   V[145]=V[144]/(1+V[144]);
   if(!finite(V[145]) || FError) return 145;
   V[146]=atan(-2*V[9]*(V[10]-V[15]/(V[113]))/(pow(V[66],2)-pow(V[65],2)))/(2);
   if(!finite(V[146]) || FError) return 146;
   V[147]=atan(-2*V[9]*(1-V[145])*(V[11]-V[15]*V[113])/(pow(V[64],2)-pow(V[63],2)))/(2);
   if(!finite(V[147]) || FError) return 147;
   V[148]=atan(-2*V[133]*(V[10]-V[15]/(V[113]))/(pow(V[70],2)-pow(V[69],2)))/(2);
   if(!finite(V[148]) || FError) return 148;
   V[149]=atan(-2*V[9]*(1-V[145])*(V[11]-V[15]*V[113])/(pow(V[68],2)-pow(V[67],2)))/(2);
   if(!finite(V[149]) || FError) return 149;
   V[150]=cos(V[146]);
   if(!finite(V[150]) || FError) return 150;
   V[151]=sin(V[146]);
   if(!finite(V[151]) || FError) return 151;
   V[152]=-sin(V[146]);
   if(!finite(V[152]) || FError) return 152;
   V[153]=cos(V[146]);
   if(!finite(V[153]) || FError) return 153;
   V[154]=cos(V[147]);
   if(!finite(V[154]) || FError) return 154;
   V[155]=sin(V[147]);
   if(!finite(V[155]) || FError) return 155;
   V[156]=-sin(V[147]);
   if(!finite(V[156]) || FError) return 156;
   V[157]=cos(V[147]);
   if(!finite(V[157]) || FError) return 157;
   V[158]=cos(V[148]);
   ...
   if(!finite(V[166]) || FError) return 166;
   V[167]=MbRun(sqrt(V[71]*V[72]));
   if(!finite(V[167]) || FError) return 167;
   ...
   if(!finite(V[255]) || FError) return 255;
   V[256]=cimag(HggA(pow(V[6]/(2)/(V[52]),2)));
   if(!finite(V[256]) || FError) return 256;
   ...
   if(!finite(V[374]) || FError) return 374;
   V[375]=1+11/(double)((4))*V[192];
   if(!finite(V[375]) || FError) return 375;
   V[376]=1+9/(double)((2))*V[192];
   if(!finite(V[376]) || FError) return 376;
   V[377]=1/(137.036);
   if(!finite(V[377]) || FError) return 377;
   V[378]=2/(double)((3));
   if(!finite(V[378]) || FError) return 378;
   V[379]=-1/(double)((3));
   if(!finite(V[379]) || FError) return 379;
   V[380]=1+V[192]*(221/(double)((12))+V[192]*(171.5-5*V[373]));
   if(!finite(V[380]) || FError) return 380;
   V[381]=V[309]*(1+V[114])/(1-V[114]*V[120]/(V[121])/(V[113]));
   if(!finite(V[381]) || FError) return 381;
   V[382]=V[313]*(1+V[114])/(1+V[114]*V[121]/(V[120])/(V[113]));
   if(!finite(V[382]) || FError) return 382;
   V[383]=V[367]*(1+V[114])/(1-V[114]/(pow(V[113],2)));
   if(!finite(V[383]) || FError) return 383;
   V[384]=V[311]*(1+V[115])/(1-V[115]*V[120]/(V[121])/(V[113]));
   if(!finite(V[384]) || FError) return 384;
   V[385]=V[315]*(1+V[115])/(1+V[115]*V[121]/(V[120])/(V[113]));
   if(!finite(V[385]) || FError) return 385;
   V[386]=V[369]*(1+V[115])/(1-V[115]/(pow(V[113],2)));
   if(!finite(V[386]) || FError) return 386;
   ...
   if(!finite(V[387]) || FError) return 387;
   ...
   if(!finite(V[388]) || FError) return 388;
   ...
   if(!finite(V[389]) || FError) return 389;
   ...
   if(!finite(V[392]) || FError) return 392;
   V[393]=-V[7]/(V[2])/(V[40])/(2);
   if(!finite(V[393]) || FError) return 393;
   V[394]=V[7]/(V[40])/(V[2])/(2);
   if(!finite(V[394]) || FError) return 394;
   V[395]=-V[192]/(8)*1/(double)((2))*sqrt(V[372])*V[393]*cabs((V[197]+I*V[198]+V[193]+I*V[194])*V[375]+(V[201]+I*V[202])*V[374]);
   if(!finite(V[395]) || FError) return 395;
   ...
   if(!finite(V[396]) || FError) return 396;
   if(VV==NULL) 
